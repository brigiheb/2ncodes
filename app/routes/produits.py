...
def get_products_by_sous_category(sous_category_id):
    ...
    try:
        ...
        query = query.filter(Produit.etat == 'actif')
        query = query.join(DureeAvecStock).filter(DureeAvecStock.quantite > 0)

        if search:
            query = query.filter(Produit.name.ilike(f'%{search}%'))

        # keep lightweight, existing sorting (by prix_1 or latest)
        if sort == 'price_asc':
        ...
        # Preload GestPrix for the responsible admin only
        # Map key: (produit_name_lower, duree_lower) -> (prix_vente, prix_achat)
        ...
        for product in products:
            ...
            if caller.role == 'admin':
                # Admin sees full duree_avec_stock + prix_affiche
                das_entries = []
                for d in product.duree_avec_stock:
                    if d.quantite <= 0:
                        continue
                    prix_achat = _prix_achat_for_admin_niveau(d, pricing_admin)
                    d_full = d.to_dict()  # contains prix_1/2/3 etc. (visible to admin)
                    d_full["prix_affiche"] = prix_achat
                    das_entries.append(d_full)
                pdata["duree_avec_stock"] = das_entries

            else:
                # Revendeur sees only slim entries: duree, quantite, prix_affiche
                das_entries = []
                for d in product.duree_avec_stock:
                    if d.quantite <= 0:
                        continue
                    ...
                    if prix_vente and float(prix_vente) > 0:
                        ...
                        # final fallback: compute from admin niveau
                        prix_affiche = _prix_achat_for_admin_niveau(d, pricing_admin)

                    # slim payload for revendeur (NO prix_1/2/3 or internals)
                    das_entries.append({
                        "duree": d.duree,
                        "Quantite": d.quantite,
                        "prix_affiche": prix_affiche
                    })
                # also slim the product, keep essential fields
                pdata = {
                    "id": product.id,
                    "name": product.name,
                    "photo": product.photo,
